...
int main() {
  ...
  Rect crop(Point(30, 30), Point(depth->width - 30, depth->height - 180));
  ...
  while(true) {
    ...
    if (foundSquare.width == 0) {
      ...
      for(size_t i = 0; i < squares.size(); i++) {
        ...
        box = cimg(bounding_rect);
        minMaxLoc(box, &min, &max);

        std::ostringstream stringStream;
        stringStream << "MIN: " << min << " MAX: " << max << '\0';

        string title = stringStream.str();

        namedWindow(title, WINDOW_NORMAL);
        imshow(title, newImage);
    ...
    if (foundSquare.width == 0) continue;

    Mat scaledBox, newMat, flippedMat;
    ...
    resize(newMat, newMat, cvSize(1920, 1080));
